__kernel void
ahomatch(__global int *trans, __global ushort16 *data, __global int *indices, 
...
{
#define ALPHABET_SIZE	2048 
//#define ALPHABET_SIZE	16384 
#define CEILDIV(x, y) (((x) + (y) - 1) / (y))
	...
	size = CEILDIV(size, 16);

	/* fetch 8 ushorts */
	for (i = 0; i < size; i++) {
		c16 = *(data + index/16 + i);
		p_c16 = (unsigned short *)&c16; 

		/* loop on the fetched data */
		for (j = 0; j < 16 / sizeof(ushort); j++) {
			c = p_c16[j * sizeof(ushort)];
//			if (c > 2047)
//			continue;
			state = *(trans + (unsigned long)(ALPHABET_SIZE * 2) *
			    (unsigned long)state + (unsigned long)c);

//			printf("(index %d) id %d i %d j %d token %hu state %d\n", index, id, i, j, c, state);
	 ...
	 * different threads
	 */
	size += (CEILDIV(max_pat_size, sizeof(ushort16)));

	/* fetch 8 ushorts */
	for ( ; i < size; i++) {

		/* guard the end of data buffer */
		if (i * 16 + index + 16 > data_size)
			return;

		c16 = *(data + index / 16 + i);
		p_c16 = (unsigned short *)&c16; /*XXX*/
